...
qcd_options * qcd_arg_parse(int argc, char* argv[], int argset_flag)
{
    ...
    strbuf help_msg,ver_msg,verb_msg,save_rs_msg,load_rg_msg,noplot_msg,   \
    latspac_fm_msg,qcomp_msg,spec_msg,part_msg,channel_msg,ss_msg,binsize_msg,\
    minimizer_msg,manf_msg;
    strbuf defmin;
    ...
    if (argset_flag & A_QCOMP)
    {
        strcpy(opt->qcomp_str,a_qcomp->sval[0]);
